...
# Checkpoint Setup
CHECKPOINT_DIR="/var/lib/k8s-setup"
CHECKPOINT_FILE="$CHECKPOINT_DIR/checkpoint_state"
mkdir -p "$CHECKPOINT_DIR"
touch "$CHECKPOINT_FILE"

# Function to check if a step is complete
is_step_complete() {
  grep -qFx "$1" "$CHECKPOINT_FILE"
}
...
if is_step_complete "Phase 1: Node Preparation"; then
  log_step "Phase 1: Node Preparation (SKIPPED - Already Complete)"
else
...
EOF
  ...
  mark_step_complete "Phase 1: Node Preparation"
fi
...
# --- Control Plane Setup ---
if is_step_complete "Phase 2: Control Plane Setup"; then
  log_step "Phase 2: Control Plane Setup (SKIPPED - Already Complete)"
  # Ensure kubeconfig is exported for later steps even if skipped
  mkdir -p $HOME/.kube
  if [ -f /etc/kubernetes/admin.conf ]; then
     # Force copy again just in case
  ...
  mark_step_complete "Phase 2: Control Plane Setup"
fi
...
if is_step_complete "Phase 3: Deploy Calico CNI"; then
  log_step "Phase 3: Deploy Calico CNI (SKIPPED - Already Complete)"
else
  ...
  mark_step_complete "Phase 3: Deploy Calico CNI"
fi
...
if is_step_complete "Phase 4: Setup Longhorn Storage"; then
  log_step "Phase 4: Setup Longhorn Storage (SKIPPED - Already Complete)"
else
  ...
  mark_step_complete "Phase 4: Setup Longhorn Storage"
fi
...
if is_step_complete "Phase 5: Setup Metrics Server"; then
  log_step "Phase 5: Setup Metrics Server (SKIPPED - Already Complete)"
else
  ...
  mark_step_complete "Phase 5: Setup Metrics Server"
fi
...
if is_step_complete "Phase 6: Install cert-manager"; then
  log_step "Phase 6: Install cert-manager (SKIPPED - Already Complete)"
else
  ...
  mark_step_complete "Phase 6: Install cert-manager"
fi

# --- Deploy MetalLB Load-Balancer (Conditional) ---
if [[ "$ENABLE_METALLB" == true ]]; then
  if is_step_complete "Phase 7: Deploy MetalLB Load-Balancer"; then
    log_step "Phase 7: Deploy MetalLB Load-Balancer (SKIPPED - Already Complete)"
  else
...
EOF
    ...
    mark_step_complete "Phase 7: Deploy MetalLB Load-Balancer"
  fi
else
...
if is_step_complete "Phase 8: Deploy PostgreSQL for Rancher"; then
  log_step "Phase 8: Deploy PostgreSQL for Rancher (SKIPPED - Already Complete)"
else
  ...
  mark_step_complete "Phase 8: Deploy PostgreSQL for Rancher"
fi
...
if is_step_complete "Phase 9: Setup Rancher Web UI"; then
  log_step "Phase 9: Setup Rancher Web UI (SKIPPED - Already Complete)"
else
  ...
  mark_step_complete "Phase 9: Setup Rancher Web UI"
fi
...
if is_step_complete "Phase 10: Install Nginx Ingress Controller"; then
  log_step "Phase 10: Install Nginx Ingress Controller (SKIPPED - Already Complete)"
else
  ...
  mark_step_complete "Phase 10: Install Nginx Ingress Controller"
fi
echo "----------------------------------------------------"
